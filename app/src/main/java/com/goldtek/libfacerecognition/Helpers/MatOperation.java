...
public class MatOperation {
    ...
    private static int FONT_SIZE = 1;//5;
    private static int THICKNESS = 2;//3;
    ...
    public static void setThickness(int nThick){
        THICKNESS = nThick;
    }
    ...
    public static Point drawRectangleOnPreview2(Mat img, Rect face, boolean front_camera, Scalar color){
        if(front_camera){
            Rect mirroredFace = getMirroredFaceForFrontCamera(img, face);
            Imgproc.rectangle(img, mirroredFace.tl(), mirroredFace.br(), color, THICKNESS);
            return mirroredFace.tl();
        } else {
            Imgproc.rectangle(img, face.tl(), face.br(), color, THICKNESS);
            return face.tl();
        }
    }
    ...
    public static void drawRectangleAndLabelOnPreview(Mat img, Rect face, String label, boolean front_camera){
        ...
        if(label.compareTo(UNKNOWN) == 0)
            color = FACE_RECT_COLOR_RED;
        else
            color = FACE_RECT_COLOR;

        Point tl = drawRectangleOnPreview2(img, face, front_camera, color);
        Imgproc.putText(img, label, tl, Core.FONT_HERSHEY_TRIPLEX, FONT_SIZE, color, THICKNESS );
    }

    public static void drawRectangleAndLabelOnPreview2(Mat img, Rect face, String label, boolean front_camera){
        Point tl = drawRectangleOnPreview(img, face, front_camera);
        //Imgproc.putText(img, label, tl, Core.FONT_HERSHEY_PLAIN, FONT_SIZE, FACE_RECT_COLOR, THICKNESS);
        Imgproc.putText(img, label, tl, Core.FONT_HERSHEY_TRIPLEX, FONT_SIZE, FACE_RECT_COLOR_RED, THICKNESS );
    }
