...
package com.goldtek.libfacerecognition.Recognition;

import android.content.Context;
import android.util.Log;

import com.goldtek.libfacerecognition.Helpers.FileHelper;
import com.goldtek.libfacerecognition.Helpers.OneToOneMap;
...
public class SupportVectorMachine implements Recognition {
    ...
    private String TAG = "SupportVectorMachine";
...
//
//        saveToFile();
        trainProbability(svmTrainOptions);
    ...
    public String recognize(Mat img, String expectedLabel) {
        ...
        return recognizeProbability(line);
...
//        return null;
    ...
    public String recognizeProbability(String svmString){
        ...
        try {
            ...
            String[] splitStr = probability.trim().split("\\s+");
            int iLabel = -1;
            String szProb = "";
            for(int i=1;i<splitStr.length;i++)
            {
                Float prob = Float.parseFloat(splitStr[i]) * 100.0f;
                int nPercen = Math.round(prob);
                if(nPercen > 50) {
                    iLabel = i;
                    szProb = String.valueOf(nPercen) + "%";
                }
            }
            ...
            if(iLabel <= 0)
                ...
                Log.i(TAG, "LabelMap: " + labelMap.getKey(iLabel) + " : " + String.valueOf(iLabel));
                ret = labelMap.getKey(iLabel) + " " + szProb;
            }
            return ret;
        } catch (FileNotFoundException e) {
            e.printStackTrace();
    ...
    private String imageToSvmString(Mat img, String label){
        ...
        if(method == TRAINING){
            if (labelMap.containsKey(label)) {
                ...
                Log.i(TAG, "LabelMap: " + label + " : " + String.valueOf(iLabel));
            }
        } else {
